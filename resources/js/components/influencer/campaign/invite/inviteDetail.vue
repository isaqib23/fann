            ...
            <v-flex xs12 md12>
                <v-card tile class="py-3">
                    <v-card-actions class="pr-12">
                        ...
                        <v-btn class="text-capitalize" color="success" depressed height="42" @click="acceptInvitation()">Accept Invitation</v-btn>
                        <v-btn class="text-capitalize" color="primary" depressed height="42" @click="rejectInvitation()">Reject Invitation</v-btn>
                    </v-card-actions>
                </v-card>
            </v-flex>
        </v-layout>
    ...
    export default {
        ...
        methods: {
            ...mapActions({
                ...
                rejectCampaignInvite        : 'campaign/rejectCampaignInvite',
                acceptCampaignInvite        : 'campaign/acceptCampaignInvite'
            }),
            reserve () {
                this.loading = true
            ...
            acceptInvitation() {
                let payload =  {
                    ...
                    sender_id    : this.$router.currentRoute.params.sender,
                    invite_id    : this.$router.currentRoute.params.invite

                };
                let response = this.acceptCampaignInvite(payload);
                if(response) {
                    this.$toast.success('You have successfully accepted campiagn invitation');
                    this.$router.push({name: 'influencer-manage-campaign'})
                }
            },
            rejectInvitation() {
                let payload =  {
                ...
                let response = this.rejectCampaignInvite(payload);
                if(response) {
                    this.$toast.success('You have successfully rejected campiagn invitation');
                    this.$router.push({name: 'influencer-campaign-invites'})
                }
            }
        },
        computed: {
