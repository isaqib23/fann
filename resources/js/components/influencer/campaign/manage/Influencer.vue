        ...
        <v-flex xs12 md7 class="pa-2">
            ...
            >
                ...
                <template v-slot:item.engRate="{ item }">
                    <div class="mb-0">
                        <span>
                            {{placementStatistics(item.placement_id,item.assign_to.statistics,'eng_rate')}}%
                        </span>
                    </div>
                </template>
                ...
                <template v-slot:item.comments="{ item }">
                    <div class="mb-0">
                        <span>
                            {{placementStatistics(item.placement_id,item.assign_to.statistics,'comment_count')}}
                        </span>
                    </div>
                </template>
                ...
                <template v-slot:item.likes="{ item }">
                    <div class="mb-0">
                        <span>
                            {{placementStatistics(item.placement_id,item.assign_to.statistics,'like_count')}}
                        </span>
                    </div>
                </template>
    ...
    export default {
        ...
        async mounted() {
            this.influencerTouchPoint = await this.getInfluencerAssignTouchPoint({
                campaign_id :   this.$router.history.current.params.slug,
